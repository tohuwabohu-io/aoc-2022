package io.tohuwabohu.aoc2022.day04
...
fun part01(): Long {
    return prepare()!!
        .filter { (it.min1 <= it.min2 && it.max1 >= it.max2) || (it.min2 <= it.min1 && it.max2 >= it.max1)}
        .count()
}
...
fun part02(): Long {
    return prepare()!!
        .filter { it.min2 in it.min1..it.max1 || it.min1 in it.min2..it.max2 }
        .count()
}

private fun prepare(): Stream<CleanupPlan>? {
    val reader = io.tohuwabohu.aoc2022.util.getBufferedReader("src/main/resources/aoc04.txt")

    return reader.lines().map { it.split(',') }
        .map { it[0].split('-') + it[1].split('-') }
        .map { it.map { str -> str.toLong() } }
        .map { CleanupPlan(min1 = it[0], max1 = it[1], min2 = it[2], max2 = it[3])}
}

private class CleanupPlan(val min1: Long, val max1: Long, val min2: Long, val max2: Long)