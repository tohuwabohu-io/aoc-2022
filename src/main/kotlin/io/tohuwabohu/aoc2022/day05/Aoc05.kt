package io.tohuwabohu.aoc2022.day05

private val list = io.tohuwabohu.aoc2022.util.getBufferedReader("src/main/resources/aoc05.txt").lines().toList()
 ...
 */
fun part01(): String {
    val crates = getInitialStack(list.filter { !it.startsWith("move") && it.isNotEmpty() }.reversed().asSequence())
    val moveCommands = getMoveCommands(list.filter { it.startsWith("move") }.asSequence())

    moveCommands.forEach { command ->
        val paletteFrom = crates[command.from]!!
        val paletteTo = crates[command.to]!!

        for (i in 1..command.amount) {
            paletteTo.add(paletteFrom.removeLast())
        }
    }

    return crates.values.map { it.last() }.toList().joinToString("")
}
...
fun part02(): String {
    ...
    moveCommands.forEach { command ->
        ...
        val cratesMoved = paletteFrom.subList(paletteFrom.size - command.amount, paletteFrom.size).toList()
        paletteTo.addAll(cratesMoved)

        crates[command.from] = paletteFrom.subList(0, paletteFrom.size - command.amount)
    }

    return crates.values.map { it.last() }.toList().joinToString("")
}

private fun getInitialStack(lines: Sequence<String>): MutableMap<Int, MutableList<Char>> {
    val map = hashMapOf<Int, MutableList<Char>>()

    lines.forEach {
        var index = 1

        it.filterIndexed { i, c -> (i == 1 || i % 4 == 1) && c !in '1'..'9' }.forEach { c ->
            var palette = map[index]
            if (palette == null) {
...
private fun getMoveCommands(lines: Sequence<String>): List<MoveCommand> {
    return lines
        .map { it.replace("move ", "").replace(" from ", ",").replace(" to ", ",") }
        .map { it.split(',') }
        .map { it.map { str -> str.toInt() } }
        .map { MoveCommand(amount = it[0], from = it[1], to = it[2]) }
        .toList()
}

private class MoveCommand(val amount: Int, val from: Int, val to: Int)