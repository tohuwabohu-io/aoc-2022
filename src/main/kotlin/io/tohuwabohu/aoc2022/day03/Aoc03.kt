package io.tohuwabohu.aoc2022.day03
 ...
 */
fun part01(): Long {
    val reader = io.tohuwabohu.aoc2022.util.getBufferedReader("src/main/resources/aoc03.txt")

    return reader
        .lines()
        .map { arrayOf(it.substring(0, it.length / 2).toSet(), it.substring(it.length / 2, it.length).toSet()) }
...
fun part02(): Long {
    ...
    return reader
        .lines()
        .map { it.substring(0, it.length / 2).toSet() + it.substring(it.length / 2, it.length).toSet() }
        .asSequence().chunked(3)
        .flatMap { it.component1().intersect(it.component2().intersect(it.component3())) }
        .map { valueMap[it]}
        .sumOf{ it!! }
}

private fun buildPriorityMap(): HashMap<Char, Long> {
