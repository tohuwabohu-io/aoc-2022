package io.tohuwabohu.aoc2022.day07
...
fun part02(): Long {
    ...
    val threshold: Long = 30000000

    val root = parse()
    val unusedSpace = totalSpace - root.size()
    val neededSpace = threshold - unusedSpace

    return root.findSmallest(neededSpace)!!.size()
}

private fun parse(): Directory {
    val reader = io.tohuwabohu.aoc2022.util.getBufferedReader("src/main/resources/aoc07.txt")

    val rootDirectory = Directory(ROOT, null, mutableListOf(), mutableListOf())
    ...
    reader.lines().forEach {
        if (it.startsWith("$")) {
            command = parseCommand(line = it)
        } else if (it.startsWith("dir")){
            val dir = parseDirectory(line = it, currentDirectory)

            if (command!!.keyword == "ls") currentDirectory.subDirs.add(dir)
        } else {
            val file = parseFile(line = it)

            if (command!!.keyword == "ls") currentDirectory.files.add(file)
        }

        if (command != null && command!!.keyword == "cd") {
            if (command!!.target != ROOT && command!!.target != UP) {
                currentDirectory = currentDirectory.subDirs.first { dir -> dir.name == command!!.target }
...
private fun parseCommand(line: String): Command {
    ...
    val keyword = split[1]
    val target = if (split.size == 3) split[2] else  ""

    return Command(keyword, target)
}

private fun parseFile(line: String): File {
    val split = line.split(" ")

    return File(split[0].toLong(), split[1])
}

private fun parseDirectory(line: String, parent: Directory): Directory {
    val split = line.split(" ")

    return Directory(name = split[1], parent, mutableListOf(), mutableListOf())
}

private class Command(val keyword: String, val target: String)

private class File(val size: Long, val name: String)

private class Directory(val name: String, val parent: Directory?, val subDirs: MutableList<Directory>, val files: MutableList<File>) {
    fun size(): Long {
        return files.sumOf { file -> file.size } + subDirs.sumOf{ dir -> dir.size() }
    }

    fun sumOf(maxSize: Long): Long {
        var sum = subDirs.sumOf { it.sumOf(maxSize) }
    ...
    fun findSmallest(minSize: Long): Directory? {
        val smallestSub = subDirs
            .filter{ it.size() > minSize }
            .map { it.findSmallest(minSize) }
            .minByOrNull { it?.size() ?: minSize }

        if (smallestSub != null) {
            return if (smallestSub.size() < size()) {
                smallestSub
            } else {
                this
            }
