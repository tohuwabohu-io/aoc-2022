package io.tohuwabohu.aoc2022.day01
 ...
 */
private var elfCaloriesMap: Map<Number, BigDecimal>? = null;
 ...
 */
fun part01(): BigDecimal {
    val caloriesSorted = getCaloriesList();

    return caloriesSorted[0]
}
...
fun part02(): BigDecimal {
    return getCaloriesList()
        .subList(0, 3)
        .fold(BigDecimal.ZERO, BigDecimal::add)
}


private fun getCaloriesList(): List<BigDecimal> {
    if (elfCaloriesMap == null) {
        val reader = io.tohuwabohu.aoc2022.util.getBufferedReader("src/main/resources/aoc01.txt");

        val caloriesPerElf = mutableMapOf<Number, BigDecimal>()
        var elfNumber = 0;
        ...
        for (line in reader.lineSequence().iterator()) {
            if ("" == line) {
                ...
                var calories = caloriesPerElf[elfNumber]
        ...
        elfCaloriesMap = caloriesPerElf;
    }

    return elfCaloriesMap!!.values.toMutableList().sortedDescending()
}
