package io.tohuwabohu.aoc2022.day06
 ...
 */
fun part01(): Int {
    val reader = io.tohuwabohu.aoc2022.util.getBufferedReader("src/main/resources/aoc06.txt")
...
private fun findMarkerIndex(line: String, bufferLen: Int): Int {
    var firstMarkerIndex = -1;

    for (i in bufferLen..line.length) {
        val distinctChars = line.subSequence(i, i + bufferLen).toSet()

        if (distinctChars.size == bufferLen) {
            firstMarkerIndex = i + bufferLen // end of sequence counts as index
            break;
        }
    }
